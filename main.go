...
// extractPDFUrls takes an HTML string and returns all the PDF URLs it finds
func extractPDFUrls(htmlInput string) []string {
	// Compile a regex that matches href="..."
	hrefRegex := regexp.MustCompile(`href="([^"]+)"`)

	// Find all matches in the input HTML
	allMatches := hrefRegex.FindAllStringSubmatch(htmlInput, -1)
	...
	for _, match := range allMatches {
		// match[0] is the full href="...", match[1] is just the URL
		if len(match) > 1 {
			currentURL := match[1]

			// Only keep URLs that contain ".pdf" (case-insensitive)
			if strings.Contains(strings.ToLower(currentURL), ".pdf") {
				pdfURLs = append(pdfURLs, currentURL)
			}
		}
	}

	// Return the list of PDF URLs
	return pdfURLs
}
...
func urlToFilename(rawURL string) string {
	...
	// Remove any query params (split on "?" and take the first part)
...
func main() {
	...
	remoteURL := "https://qualitycarcareproducts.net/pages/sds-gliptone"
	...
	for _, urls := range extractedPDFURLs {
		if !hasDomain(urls) {
			urls = "https://qualitycarcareproducts.com" + urls
